...
namespace RealisticEyeMovements
{
	...
	public class HeadComponent
	{
		#region fields
			...
			public float currentHeadIKWeight { get; private set; }
		...
		{
			float normalizedHeadAngle = Utils.NormalizedDegAngle(headAngle);
			float absAngle = Mathf.Abs(normalizedHeadAngle);

			return Mathf.Sign(normalizedHeadAngle) * Mathf.Min(eyeAndHeadAnimator.maxHeadYawAngle, absAngle);
		}
		...
		float ClampVerticalHeadAngle(float headAngle)
		{
			float normalizedHeadAngle = Utils.NormalizedDegAngle(headAngle);
			float absAngle = Mathf.Abs(normalizedHeadAngle);
			bool isUp = normalizedHeadAngle < 0;

			float maxAngle = isUp ? eyeAndHeadAnimator.maxHeadPitchUpAngle : eyeAndHeadAnimator.maxHeadPitchDownAngle;
			return Mathf.Sign(normalizedHeadAngle) * Mathf.Min(maxAngle, absAngle);
		}
		...
		{
			Vector3 spineUp = (headXform.position - spineBaseXform.position).normalized;
			float angleBetweenSpineAndWorldUp = Vector3.Angle(Vector3.up, spineUp);
			Vector3 up = angleBetweenSpineAndWorldUp < eyeAndHeadAnimator.useUpInsteadOfSpineAngle ?
				Vector3.up : spineUp;
		...
		public void Initialize(EyeAndHeadAnimator eyeAndHeadAnimator, Animator animator, bool areRotationOffsetsSet)
		{
			this.eyeAndHeadAnimator = eyeAndHeadAnimator;
				...
				if ( lookAtIK != null && headControl == HeadControl.FinalIK )
					lookAtIK.solver.IKPositionWeight = eyeAndHeadAnimator.mainWeight * eyeAndHeadAnimator.headWeight;
			#endif
		...
		void InitializeHeadXform(EyeAndHeadAnimator eyeAndHeadAnimator, Animator animator)
		{
			...
			{
				if ( headXform == null && animator != null && Utils.GetHumanoidBone(animator, HumanBodyBones.Head) != null )
					headXform = Utils.GetHumanoidBone(animator, HumanBodyBones.Head);
			...
			{ 
				...
				if ( neckXform == null && animator != null )
					neckXform = Utils.GetHumanoidBone(animator, HumanBodyBones.Neck);
				...
				if ( headXform == null )
				{
					if ( animator.isHuman )
						Debug.LogError( eyeAndHeadAnimator.name + ": " + (headControl == HeadControl.Transform
								...
								: "RealisticEyeMovements: head control is set to Head Target, but no head target assigned! Reverting to no head control."),
		...
		public float LimitHorizontalHeadAngleSoftly( float headAngle )
		{
			...
			return Utils.AsymptoticClamp(normalizedHeadAngle, -eyeAndHeadAnimator.maxHeadYawAngle, eyeAndHeadAnimator.maxHeadYawAngle);
		}
		...
		public float LimitVerticalHeadAngleSoftly( float headAngle )
		{
			...
			return Utils.AsymptoticClamp(normalizedHeadAngle, -eyeAndHeadAnimator.maxHeadPitchUpAngle, eyeAndHeadAnimator.maxHeadPitchDownAngle);
		}
		...
		void UpdateHeadEffector(float deltaTime)
		{
			...
			if ( useHillIfPossible &&  headDuration > 0 && headDuration >= timeSinceHeadMovementStart && maxHillSmoothLerpDuringThisHeadMovement < 1 && false == IsHeadMovementTotalAngleTooSmall() )
			{
